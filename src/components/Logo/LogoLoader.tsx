...
/**
 * Animated brand mark: the ©-monogram draws its outline, fills in, then fades
 * out and redraws in a loop (see `.logo-loader` in globals.css). On-brand
 * loading indicator for intentional, longer-lived loading states - not the
 * route Suspense fallback, where fast navigations make it flicker mid-draw.
 * Reduced-motion users get the static filled mark.
 */
export const LogoLoader = ({ className }: Props) => (
  <svg
    ...
    role="img"
    aria-label="Zwicker"
    className={cn('logo-loader h-auto w-44 md:w-64 lg:w-72', className)}
  >
    <path
      pathLength={1}
      className="logo-loader__path"
